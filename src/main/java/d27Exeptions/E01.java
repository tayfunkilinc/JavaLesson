...
public class E01 {
    public static void main(String[] args) {
        ...
        //division(5,0); // ArithmeticException HATA si karsimiza cikar

        //division2(7,0); //Lutfen Sifira Bolme Islemi Ypamayiniz --> burda try catch bloklariyla hatayi duzenledik
        division3(8,0);

        }//Main disi
    public static void division (int a, int b){
        if(b == 0){
            System.out.println("Payda Sifir Olamaz"); //Payda Sifir Olamaz
        }else {
            System.out.println(a /b );
        }
    }
    // bu sekilde tek tek olusabilecek hatalri tespit etmek zordur bunu try catch bloklariyla yapalim


    public static void division2 (int a, int b){
        try{
            System.out.println(a/b); // burda hata olursa catch bu hatayi yakalar
            System.out.println("try ici gorev tamamlandi"); // burasi calismadi cunku exeption firlatildigi an calisma bir ust satirda durduruldu ve bu exeption i catch a yakalar
        }
        catch (ArithmeticException e){ // e degisken ismi ArithmeticException matamatikteki tum hatalari yakalar
            System.out.println("Lutfen Sifira Bolme Islemi Yapamayiniz " + e);
        }
        System.out.println("try-catch sonrasi kod akisina devam ediyor"); //burasi try-catch sonrasi calismaya devam eder uygulamamiz calismaya devam etti kilitlenmenin onune gectik
    ...
    public static void division3(int a, int b) {
        ...
        try {
            if (b == 0) {
                throw new ArithmeticException("Sifira bolme hatasi"); // burda kendimizde bir exeption firlatabiliyoruz
            }
        } catch (ArithmeticException e) {
            System.out.println("Lutfen sifira bolme islemi yapmayiniz " + e); //Lutfen sifira bolme islemi yapmayiniz java.lang.ArithmeticException: Sifira bolme hatasi
        }
